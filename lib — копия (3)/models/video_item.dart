...
class VideoItem extends Equatable {
  ...
  /// IMDb id (формат `tt1234567`) — нужен для извлечения потока через
  /// vidsrc.xyz / 2embed (там IMDb стабильнее TMDB). Не сохраняется в БД,
  /// проставляется только когда фильм пришёл из каталога Kinopoisk.
  final String? imdbId;

  /// Прямой URL файла английских субтитров (.srt/.vtt). Используется
  /// для источников без TMDB id (например, Internet Archive), где
  /// OpenSubtitles не сработает.
  final String? subtitleUrl;

  /// То же для русских субтитров.
  final String? subtitleUrlRu;
  ...
  VideoItem copyWith({
    ...
    return VideoItem(
      ...
      subtitleUrlRu: subtitleUrlRu ?? this.subtitleUrlRu,
